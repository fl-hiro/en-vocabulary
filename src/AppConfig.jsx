...
function AppConfig() {
  ...
  const changeVoiceDetailConfig = (e) => {
    ...
    const lsKey =
      radioValue === "male" || radioValue === "female"
        ? "voice_gender"
        : "voice_speed";
    const setVoiceDetailSelect = { ...selected };
    const stateKey =
      radioValue === "male" || radioValue === "female" ? "gender" : "speed";
  ...
  const setInitialConfigOutline = (configValue) => {
    ...
    const voiceConfigValue =
      !configValue || configValue === null ? "off" : configValue.voice;

    const setConfigObjValue = { ...configStutas };
    questionConfigValue === "type01"
      ? (setConfigObjValue.display_type = false)
      : (setConfigObjValue.display_type = true);
    voiceConfigValue === "off"
      ? (setConfigObjValue.voice = false)
      : (setConfigObjValue.voice = true);
    voiceConfigValue === "off" ? "" : showDetailVoiceConfig(true);
  ...
  useEffect(() => {
    ...
    !configValue || configValue === null
      ? setInitialVoiceDetailConfig("male", "demale")
      : setInitialVoiceDetailConfig(
          configValue.voice_gender,
          configValue.voice_speed
        );
  }, []);
    ...
    <>
      <div className="config-info">
        <ul className="config-info__list">
          <li className="config-info__item">
            <input
              id="radio01"
              type="checkbox"
              className="config-info__radio"
              checked={configStutas.display_type}
              onChange={changeQuestionConfig}
            />
            <label htmlFor="radio01" className="config-info__label">
              Change displayed type of question
            </label>
          </li>
          <li className="config-info__item">
            <input
              ...
              type="checkbox"
              className="config-info__radio"
              checked={configStutas.voice}
              onChange={changeVoiceConfig}
            />
            <label htmlFor="radio02" className="config-info__label">
              Turn on voice function
            </label>
          </li>
          <li
            className="config-info__item config-info-detail"
            ref={voiceConfigGenderRef}
          >
            {genderRadio.map((item, index) => {
              return (
                <div
                  className="config-info-detail__item config-info-detail__item--col2"
                  key={index}
                >
                  <input
                    id={`genderRadio0${index}`}
                    type="radio"
                    name="gender"
                    value={item.value}
                    className="config-info-detail__radio"
                    checked={item.value === selected.gender}
                    onChange={changeVoiceDetailConfig}
                  ></input>
                  <label
                    htmlFor={`genderRadio0${index}`}
                    className="config-info-detail__label"
                  >
                    {item.label}
                  </label>
                </div>
          ...
          <li
            className="config-info__item config-info-detail"
            ref={voiceConfigSpeedRef}
          >
            {speedRadio.map((item, index) => {
              return (
                <div
                  className="config-info-detail__item config-info-detail__item--col3"
                  key={index}
                >
                  <input
                    id={`speedRadio0${index}`}
                    type="radio"
                    name="speed"
                    value={item.value}
                    className="config-info-detail__radio"
                    checked={item.value === selected.speed}
                    onChange={changeVoiceDetailConfig}
                  ></input>
                  <label
                    htmlFor={`speedRadio0${index}`}
                    className="config-info-detail__label"
                  >
                    {item.label}
                  </label>
        ...
        <div className="config-close">
          <button className="config-close__button" onClick={blowsBack}>
            Back
          </button>
        </div>
      </div>
